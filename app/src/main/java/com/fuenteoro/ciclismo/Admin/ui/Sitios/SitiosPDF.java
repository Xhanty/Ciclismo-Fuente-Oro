...
public class SitiosPDF {
    ...
    public SitiosPDF(Context context) {
    ...
    private void createFile(){
        ...
        if(!folder.exists())
            folder.mkdirs();
        pdfFile = new File(folder, "Reporte_Sitios.pdf");
    }
    ...
    public void addInfo(){
        try{
            ...
            addChild(new Paragraph("Listado de sitios", fTitle));
    ...
    public void createTable(String[]header, ArrayList<String[]>sitios, String fecha){
        try {
            paragraph = new Paragraph();
            ...
            for (int indexR = 0; indexR < sitios.size(); indexR++) {
                String[] row = sitios.get(indexR);
